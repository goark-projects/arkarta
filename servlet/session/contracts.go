...
var ErrInvalidCookieConfig = errors.New(
	"arkarta/servlet/session: invalid cookie config",
)

// ErrCookieConfigLocked 表示应用已经启动，不能再修改 Session Cookie 配置。
var ErrCookieConfigLocked = errors.New(
	"arkarta/servlet/session: cookie config is locked",
)

// ErrInvalidURLRewriteConfig 表示 URL 重写配置非法。
var ErrInvalidURLRewriteConfig = errors.New(
	"arkarta/servlet/session: invalid url rewrite config",
)
