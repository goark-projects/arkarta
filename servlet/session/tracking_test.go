...
func TestTrackingPolicyModes(t *testing.T) {
	...
	if !policy.Allows(TrackingCookie) || !policy.Allows(TrackingURL) ||
		policy.Allows(TrackingSSL) {
	...
	if _, err := NewTrackingPolicy(TrackingMode("BAD")); !errors.Is(
		err,
		ErrInvalidTrackingMode,
	) {
