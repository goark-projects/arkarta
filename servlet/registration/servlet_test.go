...
func TestServletRegistrationMappingsAndSnapshot(t *testing.T) {
	...
	config := multipart.NewConfig(
		multipart.WithMaxFileSize(1024),
		multipart.WithMaxRequestSize(4096),
		multipart.WithFileSizeThreshold(128),
	)
	...
	first := servlets[0]
	if first.Name() != "orders" || !first.AsyncSupported() ||
		first.RunAsRole() != "admin" {
		t.Fatalf(
			"servlet descriptor = %q/%v/%q, want orders/true/admin",
			first.Name(),
			first.AsyncSupported(),
			first.RunAsRole(),
		)
	}
	...
	if !ok || gotConfig.MaxFileSize() != 1024 || gotConfig.MaxRequestSize() != 4096 ||
		gotConfig.FileSizeThreshold() != 128 {
...
func TestServletRegistrationRejectsInvalidInput(t *testing.T) {
	...
	registry := registration.NewRegistry()
	if _, err := registry.AddServlet("", servlet.HandlerFunc(noopServe)); !errors.Is(
		err,
	...
	if _, err := registry.AddServlet("nil", nilHandler); !errors.Is(
		err,
		registration.ErrNilServlet,
	) {
	...
	if _, err := registry.AddServlet("orders", servlet.HandlerFunc(noopServe)); !errors.Is(
		err,
		registration.ErrDuplicateRegistration,
	) {
	...
	if _, err := orders.SetInitParam("", "bad"); !errors.Is(
		err,
		registration.ErrInvalidInitParamName,
	) {
		t.Fatalf("empty init param err = %v, want ErrInvalidInitParamName", err)
	}
	if _, err := orders.AddMapping("orders"); !errors.Is(
		err,
...
func TestRegistryFreezeRejectsFurtherMutation(t *testing.T) {
	...
	if _, err := orders.SetInitParam("encoding", "utf-8"); !errors.Is(
		err,
	...
	if _, err := registry.AddFilter("audit", servlet.FilterFunc(noopFilter)); !errors.Is(
		err,
		registration.ErrRegistryFrozen,
	) {
