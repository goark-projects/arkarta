...
type HTTPErrorPageFactory func(
	handler servlet.Handler,
	registry *servlet.ErrorPageRegistry,
) http.Handler
...
func RunErrorPages(t *testing.T, factory HTTPErrorPageFactory) {
	...
	t.Run("status_error_page", func(t *testing.T) {
		...
		if err := registry.RegisterStatus(http.StatusForbidden, servlet.HandlerFunc(
		...
		factory(
			handler,
			registry,
		).ServeHTTP(recorder, httptest.NewRequest(http.MethodGet, "/secure", nil))

		if recorder.Code != http.StatusForbidden ||
			recorder.Body.String() != "forbidden-page" {
			t.Fatalf("status/body = %d/%q", recorder.Code, recorder.Body.String())
		}
	})
	t.Run("panic_error_page", func(t *testing.T) {
		registry := servlet.NewErrorPageRegistry()
		if err := registry.RegisterStatus(http.StatusInternalServerError, servlet.HandlerFunc(
			func(_ context.Context, req *servlet.Request, res servlet.Response) error {
		...
		if recorder.Code != http.StatusInternalServerError ||
			recorder.Body.String() != "panic-page" {
			t.Fatalf("status/body = %d/%q", recorder.Code, recorder.Body.String())
		}
	})
	t.Run("default_error_page", func(t *testing.T) {
		registry := servlet.NewErrorPageRegistry()
		if err := registry.RegisterDefault(servlet.HandlerFunc(
		...
		factory(
			handler,
			registry,
		).ServeHTTP(recorder, httptest.NewRequest(http.MethodGet, "/default", nil))

		if recorder.Code != http.StatusTeapot ||
			recorder.Body.String() != "default-page" {
			t.Fatalf(
				"status/body = %d/%q, want default error page",
				recorder.Code,
	...
	t.Run("error_type_last_registration_wins", func(t *testing.T) {
		registry := servlet.NewErrorPageRegistry()
		if err := servlet.RegisterErrorType[*tckTypedError](registry, servlet.HandlerFunc(
			func(_ context.Context, _ *servlet.Request, res servlet.Response) error {
		...
		factory(
			handler,
			registry,
		).ServeHTTP(recorder, httptest.NewRequest(http.MethodGet, "/typed", nil))

		if recorder.Code != http.StatusInternalServerError ||
			recorder.Body.String() != "last" {
			t.Fatalf(
				"status/body = %d/%q, want last typed error page",
				recorder.Code,
				recorder.Body.String(),
			)
		}
	...
	t.Run("error_page_loop_is_rejected", func(t *testing.T) {
		...
		if err := registry.RegisterStatus(http.StatusInternalServerError, servlet.HandlerFunc(
			func(ctx context.Context, req *servlet.Request, res servlet.Response) error {
				nested := servlet.NewHTTPError(http.StatusInternalServerError, "nested", nil)
				handled, err := registry.Handle(
					ctx, req, res, http.StatusInternalServerError, nested,
				)
		...
		factory(
			handler,
			registry,
		).ServeHTTP(recorder, httptest.NewRequest(http.MethodGet, "/loop", nil))

		if recorder.Code != http.StatusInternalServerError ||
			recorder.Body.String() != "loop-guard" {
			t.Fatalf(
				"status/body = %d/%q, want guarded error page",
				recorder.Code,
				recorder.Body.String(),
			)
		}
