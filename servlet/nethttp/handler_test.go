...
func TestHandlerAppliesRequestOptions(t *testing.T) {
	handler := HandlerWithOptions(
		servlet.HandlerFunc(
			func(_ context.Context, req *servlet.Request, res servlet.Response) error {
				if err := req.ParseParameters(); errors.Is(
					err,
					servlet.ErrFormBodyTooLarge,
				) {
...
func TestHandlerCommitsStatusWithoutBody(t *testing.T) {
	...
	handler.ServeHTTP(
		recorder,
		httptest.NewRequest(http.MethodDelete, "/resource", nil),
	)
...
func TestHandlerSupportsResponseConvenienceHelpers(t *testing.T) {
	...
	handler := Handler(
		servlet.HandlerFunc(
			func(_ context.Context, _ *servlet.Request, res servlet.Response) error {
				cookie := &servlet.Cookie{Name: "sid", Value: "abc", HTTPOnly: true}
				if err := servlet.AddCookie(res, cookie); err != nil {
					return err
				}
	...
	if recorder.Header().Get("Set-Cookie") != "" {
		t.Fatalf(
			"cookie must be reset by Redirect, got %q",
...
func TestHandlerMapsWrappedStatusError(t *testing.T) {
	...
	handler := Handler(
		servlet.HandlerFunc(
			func(context.Context, *servlet.Request, servlet.Response) error {
				return errors.Join(
					servlet.NewHTTPError(http.StatusConflict, "conflict", nil),
