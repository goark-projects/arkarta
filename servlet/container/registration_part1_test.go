...
func TestDeploymentFromRegistrationBuildsServletAndFilterMappings(t *testing.T) {
	...
	if conflicts, err := orders.AddMapping("/orders/*"); err != nil ||
		len(conflicts) != 0 {
		t.Fatalf(
			"AddMapping orders conflicts/err = %#v/%v, want none/nil",
			conflicts,
			err,
		)
	}
	other, err := registry.AddServlet(
		"other",
		servlet.HandlerFunc(
			func(context.Context, *servlet.Request, servlet.Response) error {
	...
	if conflicts, err := other.AddMapping("/other/*"); err != nil ||
		len(conflicts) != 0 {
		t.Fatalf(
			"AddMapping other conflicts/err = %#v/%v, want none/nil",
			conflicts,
			err,
		)
	}
	...
	ordersReq, err := servlet.NewRequest(
	...
	otherReq, err := servlet.NewRequest(
		httptest.NewRequest(http.MethodGet, "/other/42", nil),
	)
	if err != nil {
