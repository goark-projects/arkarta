...
func TestDefaultValidatorValidatesGroupsAndMessages(t *testing.T) {
	...
	validator := validation.NewValidator(
		validation.WithMessageResolver(
			validation.MessageResolverFunc(
				func(_ context.Context, message validation.MessageContext) (string, bool) {
	...
	createResult, err := validator.ValidateGroups(
		context.Background(),
		payload{},
		"create",
	)
	...
	if len(createViolations) != 1 || createViolations[0].Path() != "name" ||
		createViolations[0].Message() != "name 必填" {
		t.Fatalf(
			"create violations = %#v, want only custom name required",
			createViolations,
		)
	}
	...
	if got := p1ViolationPaths(defaultResult); !reflect.DeepEqual(
		got,
...
func TestDefaultValidatorObjectConstraint(t *testing.T) {
	...
	validator := validation.NewValidator(
		validation.WithObjectConstraint(period{}, validation.ObjectConstraintFunc{
			...
			Fn: func(_ context.Context, object validation.ObjectContext) ([]validation.Violation, error) {
				...
				if value.End < value.Start {
					return []validation.Violation{
						validation.NewViolation(
							object.FieldPath("end"),
							"periodOrder",
							"结束值不能小于开始值",
							value.End,
						),
	...
	if got := p1ViolationPaths(result); !reflect.DeepEqual(
		got,
		[]string{"end:periodOrder"},
	) {
