...
当前阶段先完成 Servlet 部分，并提供 WebSocket、JSON、Validation 第一版独立标准包。Servlet 相关代码全部位于 `servlet` 包及其子包下，WebSocket 位于根级 `websocket` 包；后续 Web 和上层安全集成等标准也会各自拆包，保持职责边界清晰。
